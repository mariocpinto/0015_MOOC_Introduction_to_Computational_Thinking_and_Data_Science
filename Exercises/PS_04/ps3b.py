...
#virus = SimpleVirus(0.5,0.8)
#print virus.getMaxBirthProb()
#print virus.getClearProb()
#print virus.doesClear()
#new_virus = virus.reproduce(0.2)
#print new_virus.getMaxBirthProb()
#print new_virus.getClearProb()
#print new_virus.doesClear()
...
class Patient(object):
    ...
    def update(self):
        ...
        # TODO
        virus_list = self.getViruses()[:]
        
        # remove a virus based on the value returned by doesClear()
        for virus in virus_list:
            if virus.doesClear():
                self.getViruses().remove(virus)
        
        # Calculate virus population density
        pop_dens = len(self.getViruses())/self.getMaxPop()
        
        virus_list = self.getViruses()[:]
        
        for virus in virus_list:
            try:
                child_virus = virus.reproduce(pop_dens)
                self.getViruses().append(child_virus)
            except NoChildException:
                pass
                
        return len(self.getViruses())

#v1 = SimpleVirus(0.5,0.2)
#v2 = SimpleVirus(0.5,0.2)
#v3 = SimpleVirus(0.5,0.2)
#v4 = SimpleVirus(0.5,0.2)
#v5 = SimpleVirus(0.5,0.2)                   
#patient = Patient([v1, v2, v3, v4, v5],20)
#patient.getViruses()
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for count in range(numViruses):
            virus_list += [SimpleVirus(maxBirthProb, clearProb)]
        
        patient = Patient(virus_list, maxPop)
        
        # calculate virus count progression
    ...
    pylab.plot(range(Num_Timesteps), virus_count, label='Average over ' + str(numTrials) + ' trials')
    pylab.title('Change in Virus Count with Time')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        Resistance_Dict = self.getResistances()
        Will_Reproduce = True
        
        for drug in activeDrugs:
            try:
                if Resistance_Dict[drug] == False:
        ...
        if Will_Reproduce and random.random() < self.getMaxBirthProb() * (1 - popDensity):

            # Update resistances            
            new_resistances = self.getResistances().copy()
            for key,val in new_resistances.items():
                if random.random() < self.getMutProb():
                    new_resistances[key] = not val
            
            return ResistantVirus(self.maxBirthProb, self.clearProb, new_resistances, self.getMutProb())
            
        else:
            raise NoChildException()
            
#virus = ResistantVirus(maxBirthProb=.8, clearProb=0.05, resistances={'a':True,'b':False,'c':True}, mutProb=0.3)
#print virus.getResistances()
#print virus.getMutProb()
#print virus.isResistantTo('a')
#print virus.isResistantTo('b')
#virus.reproduce(0.5,['a', 'c'])
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.getViruses():
            ...
            for drug in drugResist:
                ...
                if drug in virus.getResistances().keys() and virus.getResistances()[drug] == True:
                    pass
                else:
                    incr_count = False
    ...
    def update(self):
        ...
        for virus in virus_list:
            try:
                child_virus = virus.reproduce(pop_dens, self.getPrescriptions())
                self.getViruses().append(child_virus)
            except NoChildException:
                pass
